...
class fk_publish_node(Node):
    def __init__(self):
        ...
        self.joint_angles = np.array([math.radians(0),math.radians(-15),math.radians(-30),math.radians(10),math.radians(-10),math.radians(0)]).astype(np.float64)
        ...
        self.current_rotation_aaxis = self.current_rotation_mat.as_rotvec()
        self.goal_orientation = self.current_rotation_mat.as_rotvec()
        print(self.current_rotation_aaxis)
        print(self.goal_orientation)
    ...
    def goal_callback(self, msg):
        if self.move == False:
            ...
            a_arr = np.linspace(self.current_rotation_aaxis[0], self.goal_orientation[0], num=1000)
            b_arr = np.linspace(self.current_rotation_aaxis[1], self.goal_orientation[1], num=1000)
            c_arr = np.linspace(self.current_rotation_aaxis[2], self.goal_orientation[2], num=1000)
            ...
            trajectory = np.column_stack((x_arr,y_arr,z_arr,a_arr,b_arr,c_arr))
            if self.iteration == 0:
                fig = plt.figure()
    ...
    def ik_iteration(self):
        if self.move == True:
            if self.iteration < len(self.trajectory):
                ...
                #                             (0.0),
                #                             (0.0),
                #                             (ref_position[3]-self.current_rotation_aaxis[0]),
                #                             (ref_position[4]-self.current_rotation_aaxis[1]),
                #                             (ref_position[5]-self.current_rotation_aaxis[2])]).astype(np.float64)
                ...
                # print((ref_position[3]-self.current_rotation_aaxis[0]),
                #     (ref_position[4]-self.current_rotation_aaxis[1]),
                #     (ref_position[5]-self.current_rotation_aaxis[2]))
